    ...
    <div className="py-16 sm:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((post, i) => (
            <motion.article
              ...
              className="group bg-ydl-card-gradient border border-ydl-dark-border rounded-2xl overflow-hidden hover:border-ydl-dark-border-hover transition-all duration-300 hover:-translate-y-1"
            >
              <div className="h-44 bg-ydl-surface-light flex items-center justify-center">
                <div className="text-center">
                  <Calendar className="w-10 h-10 text-ydl-yellow/30 mx-auto" />
                  <p className="text-xs text-ydl-muted mt-2">Featured Image</p>
                </div>
              </div>
              <div className="p-5">
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-2.5 py-0.5 bg-ydl-yellow/10 text-ydl-yellow text-[10px] font-semibold rounded-full">
                    {post.category}
                  </span>
                  <span className="text-[10px] text-ydl-muted">{post.readTime}</span>
                </div>
                <h2 className="font-heading font-bold text-white text-base mb-2 group-hover:text-ydl-yellow transition-colors line-clamp-2">
                  {post.title}
                </h2>
                <p className="text-xs text-ydl-muted leading-relaxed mb-4 line-clamp-2">
                  {post.excerpt}
                </p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-[11px] text-ydl-muted">
                    <User className="w-3 h-3" />
                    {post.author}
                    <span className="mx-1">·</span>
                    {post.date}
                  </div>
                  <span className="text-ydl-yellow text-xs font-medium group-hover:underline underline-offset-2">
                    Read More
                  </span>
                </div>
              </div>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-16 bg-ydl-card-gradient border border-ydl-dark-border rounded-2xl p-8 sm:p-12 text-center"
        >
          <h2 className="font-heading text-2xl sm:text-3xl font-bold text-white mb-3">
            Subscribe to Our Newsletter
          </h2>
          <p className="text-ydl-muted-light mb-6 max-w-lg mx-auto">
          ...
          >
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-6 py-3 text-sm font-bold text-black bg-ydl-gradient rounded-xl hover:shadow-ydl-gold-lg transition-all whitespace-nowrap"
            >
              Subscribe
            </button>
          </form>
