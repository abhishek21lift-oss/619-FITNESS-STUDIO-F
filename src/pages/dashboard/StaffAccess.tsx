  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-bold text-white">Access Control</h1>
          <p className="text-xs text-gray-500 mt-0.5">Manage individual staff permissions.</p>
        </div>
        <button onClick={savePermissions} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90 transition-opacity">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
            ...
            <div className="space-y-1 max-h-[300px] overflow-y-auto">
              {staffList.map(name => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs rounded-lg transition-colors ${selectedStaff === name ? 'bg-ydl-yellow/10 text-ydl-yellow border border-ydl-yellow/20' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}
                >
                  {name}
                </button>
              ))}
            ...
            <div className="space-y-1.5">
              {Object.keys(roleTemplates).map(role => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-xs font-medium text-gray-400 bg-white/5 border border-ydl-dark-border rounded-lg hover:bg-white/10 hover:text-white transition-colors"
                >
                  {role}
                </button>
              ))}
        ...
        <div className="lg:col-span-3">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] overflow-hidden">
            <div className="p-3 border-b border-ydl-dark-border bg-white/[0.03]">
              <h3 className="text-xs font-semibold text-white">
                Permissions for <span className="text-ydl-yellow">{selectedStaff}</span>
              </h3>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-ydl-dark-border bg-white/[0.03]">
                    <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase w-40">Module</th>
                    {perms.map(p => (
                      <th key={p} className="text-center px-2 py-3 text-[10px] font-semibold text-gray-500 uppercase">{p}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-ydl-dark-border/50">
                  {sections.map((section, i) => (
                    <motion.tr key={section} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-3 text-xs font-medium text-white">{section}</td>
                      {perms.map((_, idx) => {
                            ...
                            <button
                              ...
                              className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${
                                checked
                                  ...
                                  : 'bg-white/5 text-gray-600 border border-ydl-dark-border hover:text-gray-400'
                              }`}
                            >
      ...
      <Modal open={showConfirm} onClose={() => setShowConfirm(false)} title="Permissions Saved" size="sm">
        <p className="text-xs text-gray-400">Permissions for <span className="text-ydl-yellow font-medium">{selectedStaff}</span> have been updated successfully.</p>
        <button onClick={() => setShowConfirm(false)} className="mt-4 px-4 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90 transition-opacity">OK</button>
      </Modal>
    </div>
  )
}
