...
export default function AppSettingsPermissions() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div><h1 className="text-lg font-bold text-[#1C1C1E]">Permissions</h1><p className="text-xs text-apple-gray-500 mt-0.5">On/Off toggles for app features.</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] p-5 max-w-2xl">
        <div className="space-y-2">
          {permissions.map((p) => (
            <div key={p.key} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.03] border border-apple-gray-200">
              <div>
                <div className="flex items-center gap-2">
                  <p className="text-xs text-[#1C1C1E]">{p.name}</p>
                </div>
                <p className="text-[9px] text-apple-gray-500 mt-0.5">{p.desc}</p>
              </div>
        ...
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} className="flex-1 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">
            <Save className="w-3.5 h-3.5 inline mr-1" /> {saved ? 'Saved!' : 'Save Changes'}
          </button>
          <button onClick={() => setPermissions(prev => prev.map(p => ({ ...p, enabled: true }))) } className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Enable All</button>
          <button onClick={() => setPermissions(prev => prev.map(p => ({ ...p, enabled: false }))) } className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Disable All</button>
        </div>
      </motion.div>
    </div>
  )
