...
export default function BatchesCancelled() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-[#1C1C1E]">Cancelled Classes</h1>
        <p className="text-xs text-apple-gray-500 mt-0.5">View cancelled batch sessions.</p>
      </div>

      <div className="flex items-center gap-4">
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium text-apple-gray-400">From</label>
          <input type="date" value={filterStart} onChange={e => setFilterStart(e.target.value)} className="w-40 bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40 [color-scheme:dark]" />
        </div>
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium text-apple-gray-400">To</label>
          <input type="date" value={filterEnd} onChange={e => setFilterEnd(e.target.value)} className="w-40 bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40 [color-scheme:dark]" />
        </div>
        {(filterStart || filterEnd) && (
          <button onClick={() => { setFilterStart(''); setFilterEnd('') }} className="mt-5 px-3 py-1.5 text-[10px] font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E] transition-colors">
            Clear Filter
          </button>
        )}
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-apple-gray-200">
              ...
              <th className="text-left px-4 py-3 text-[10px] font-medium text-apple-gray-500 uppercase tracking-wider">Reason</th>
              <th className="text-left px-4 py-3 text-[10px] font-medium text-apple-gray-500 uppercase tracking-wider">Cancelled By</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((c, i) => (
              <tr key={c.id} className="border-b border-apple-gray-200/50 last:border-0 hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3 text-xs text-apple-gray-500">{i + 1}</td>
                <td className="px-4 py-3">
                  <span className="flex items-center gap-1.5 text-xs text-[#1C1C1E]">
                ...
                <td className="px-4 py-3 text-xs text-apple-gray-400">{c.time}</td>
                <td className="px-4 py-3 text-xs text-apple-gray-600">{c.reason}</td>
                <td className="px-4 py-3 text-xs text-apple-gray-400">{c.cancelledBy}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-xs text-apple-gray-500">No cancelled classes found.</td></tr>
            )}
          </tbody>
        </table>
      </motion.div>
