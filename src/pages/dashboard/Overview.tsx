...
export default function Overview() {
  ...
  const statsCards = stats ? [
    { label: "Today's Sale", value: `₹${(stats.revenueThisMonth || 0).toLocaleString()}`, icon: DollarSign, color: 'from-[#007AFF]/10 to-[#007AFF]/5', border: 'border-[#007AFF]/20', text: 'text-[#007AFF]' },
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      ...
      {/* Coupon Banner */}
      <div className="p-3 rounded-xl bg-gradient-to-r from-[#007AFF]/10 via-amber-600/10 to-[#007AFF]/5 border border-[#007AFF]/20">
        <p className="text-xs font-medium text-[#007AFF]">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-apple-blue/10">
        ...
        {dateFilters.map((f) => (
          <button
            ...
            className={`px-3 py-1 text-[10px] font-medium rounded-lg border transition-all ${
              activeFilter === f
                ? 'bg-apple-blue/10 border-[#007AFF]/30 text-[#007AFF]'
                : 'bg-white/5 border-apple-blue/10 text-apple-gray-500 hover:text-apple-gray-600 hover:border-gray-600'
            }`}
          >
            {f}
          </button>
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-apple-gray-400 py-8"><Loader2 className="w-5 h-5 animate-spin" /> Loading...</div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {metrics.map((m, i) => (
              <motion.div
                ...
                className="rounded-xl border border-apple-blue/10 bg-white/[0.02] p-4"
              >
      ...
      <motion.div
        ...
        className="rounded-xl border border-apple-blue/10 bg-white/[0.02] overflow-hidden"
      >
        <div className="px-4 py-3 border-b border-apple-blue/10">
          <h2 className="text-xs font-semibold text-[#1C1C1E]">Summary.</h2>
        </div>
        <div className="divide-y divide-[#007AFF]/10">
          {summaryRows.map((row) => (
            <div key={row.label} className="flex items-center justify-between px-4 py-2.5 hover:bg-white/[0.02] transition-colors">
              ...
              {row.href ? (
                <a href={row.href} className="text-[11px] font-medium text-[#007AFF] hover:text-[#E8C84A] underline underline-offset-2">
                  {row.value}
                </a>
              ) : (
      ...
      <div className="flex items-center justify-between pt-2 pb-4">
        ...
        <a href="#" className="text-[10px] text-apple-gray-400 hover:text-[#007AFF] transition-colors">
          Download Anydesk
        </a>
      </div>
    </div>
