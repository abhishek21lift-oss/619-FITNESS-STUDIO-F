...
const quickActions = [
  { label: 'Add Enquiry', icon: Plus, color: 'from-blue-500/20 to-blue-600/5', border: 'border-blue-500/30', text: 'text-blue-400' },
...
export default function Overview() {
  ...
  const statsCards = stats ? [
    { label: "Today's Sale", value: `₹${(stats.revenueThisMonth || 0).toLocaleString()}`, icon: DollarSign, color: 'from-blue-500/20 to-blue-600/5', border: 'border-blue-500/30', text: 'text-blue-400' },
    { label: 'Collected Payments', value: `₹${(stats.revenueThisMonth || 0).toLocaleString()}`, icon: ArrowUpRight, color: 'from-green-500/20 to-green-600/5', border: 'border-green-500/30', text: 'text-green-400' },
    { label: 'Pending Payments', value: `₹${(stats.pendingDues || 0).toLocaleString()}`, icon: AlertTriangle, color: 'from-red-500/20 to-red-600/5', border: 'border-red-500/30', text: 'text-red-400' },
  ] : [
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      ...
      {/* Coupon Banner */}
      <div className="p-3 rounded-xl bg-gradient-to-r from-[#D4AF34]/10 via-amber-600/10 to-[#D4AF34]/5 border border-[#D4AF34]/20">
        <p className="text-xs font-medium text-[#D4AF34]">
          🎉 Special Coupon Offers Available —{' '}
          <a href="#" className="underline underline-offset-2 hover:text-[#E8C84A]">Click here to activate</a>
        </p>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-[#D4AF34]/10">
          <Users className="w-3.5 h-3.5 text-[#D4AF34]" />
        ...
        {dateFilters.map((f) => (
          <button
            ...
            className={`px-3 py-1 text-[10px] font-medium rounded-lg border transition-all ${
              activeFilter === f
                ? 'bg-[#D4AF34]/10 border-[#D4AF34]/30 text-[#D4AF34]'
                : 'bg-white/5 border-[#D4AF34]/10 text-gray-500 hover:text-gray-300 hover:border-gray-600'
            }`}
          >
            {f}
          </button>
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-ydl-muted py-8"><Loader2 className="w-5 h-5 animate-spin" /> Loading...</div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {statsCards.map((card, i) => (
              ...
              >
                ...
                <a href="#" className="inline-block mt-2 text-[10px] text-gray-500 hover:text-gray-300 underline underline-offset-2">
                  View More
                </a>
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {metrics.map((m, i) => (
              <motion.div
                ...
                className="rounded-xl border border-[#D4AF34]/10 bg-white/[0.02] p-4"
              >
      ...
      <motion.div
        ...
        className="rounded-xl border border-[#D4AF34]/10 bg-white/[0.02] overflow-hidden"
      >
        <div className="px-4 py-3 border-b border-[#D4AF34]/10">
          <h2 className="text-xs font-semibold text-white">Summary.</h2>
        </div>
        <div className="divide-y divide-[#D4AF34]/10">
          {summaryRows.map((row) => (
            <div key={row.label} className="flex items-center justify-between px-4 py-2.5 hover:bg-white/[0.02] transition-colors">
              ...
              {row.href ? (
                <a href={row.href} className="text-[11px] font-medium text-[#D4AF34] hover:text-[#E8C84A] underline underline-offset-2">
                  {row.value}
                </a>
              ) : (
      ...
      <div className="flex items-center justify-between pt-2 pb-4">
        ...
        <a href="#" className="text-[10px] text-gray-600 hover:text-[#D4AF34] transition-colors">
          Download Anydesk
        </a>
      </div>
    </div>
