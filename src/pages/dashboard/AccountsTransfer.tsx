...
import { ArrowRightLeft, DollarSign, Users, Plus, Search, Download } from 'lucide-react'
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-white">Plan Transfers</h1><p className="text-xs text-gray-500 mt-0.5">Member plan change records.</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <StatsCard label="Transfers" value={data.length} icon={ArrowRightLeft} color="from-blue-500/20 to-blue-600/5" border="border-blue-500/30" text="text-blue-400" />
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Search transfers..." />
        </div>
        <div className="flex items-center gap-2">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-ydl-dark-border bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase">Transfer Fee</th>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Transfer' : 'Add Transfer'} size="md">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-gray-400">Transfer Fee (₹)</label>
              <input type="number" value={form.transferFee} onChange={e => setForm(prev => ({ ...prev, transferFee: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="0" />
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-ydl-dark-border">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90">{editItem ? 'Update' : 'Add Transfer'}</button>
