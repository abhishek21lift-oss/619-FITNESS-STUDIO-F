  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-white">Members List</h1>
          <p className="text-xs text-gray-500 mt-0.5">View and manage all registered members.</p>
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-ydl-dark-border bg-white/[0.03]">
                ...
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-gray-500 uppercase">Email</th>
                ...
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-gray-500 uppercase">Joined Date</th>
            ...
            <tbody className="divide-y divide-ydl-dark-border/50">
              {paged.map((m, i) => (
                <motion.tr key={m.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-3 py-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-ydl-yellow/10 flex items-center justify-center text-[10px] font-bold text-ydl-yellow">{m.name.split(' ').map(n => n[0]).join('')}</div><span className="text-xs font-medium text-white">{m.name}</span></div></td>
                  <td className="px-3 py-3"><div className="flex items-center gap-1.5"><Phone className="w-3 h-3 text-gray-500" /><span className="text-xs text-gray-400">{m.mobile}</span></div></td>
                  <td className="px-3 py-3 text-xs text-gray-400">{m.email}</td>
                  <td className="px-3 py-3 text-xs text-gray-400">{m.plan}</td>
                  <td className="px-3 py-3">{statusBadge(m.status)}</td>
                  <td className="px-3 py-3"><div className="flex items-center gap-1.5"><Calendar className="w-3 h-3 text-gray-500" /><span className="text-xs text-gray-400">{m.joinedDate}</span></div></td>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button disabled={page <= 1} onClick={() => setPage(page - 1)} className="p-1.5 text-gray-500 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed rounded-lg border border-ydl-dark-border bg-white/5 hover:bg-white/10 transition-colors"><ChevronLeft className="w-3.5 h-3.5" /></button>
          {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
            const start = Math.max(1, Math.min(page - 2, totalPages - 4))
            const p = start + i
            if (p > totalPages) return null
            return <button key={p} onClick={() => setPage(p)} className={`w-7 h-7 text-[10px] font-medium rounded-lg border transition-colors ${page === p ? 'bg-ydl-yellow/10 border-ydl-yellow/30 text-ydl-yellow' : 'bg-white/5 border-ydl-dark-border text-gray-500 hover:text-white'}`}>{p}</button>
          })}
          <button disabled={page >= totalPages} onClick={() => setPage(page + 1)} className="p-1.5 text-gray-500 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed rounded-lg border border-ydl-dark-border bg-white/5 hover:bg-white/10 transition-colors"><ChevronRight className="w-3.5 h-3.5" /></button>
        </div>
      </div>
        ...
        {modal?.data && (
          <div className="space-y-3 text-xs">
            <div className="flex items-center gap-3 pb-3 border-b border-ydl-dark-border">
              <div className="w-10 h-10 rounded-full bg-ydl-yellow/10 flex items-center justify-center text-base font-bold text-ydl-yellow">{modal.data.name.split(' ').map((n: string) => n[0]).join('')}</div>
              <div><h3 className="text-sm font-semibold text-white">{modal.data.name}</h3><p className="text-[10px] text-gray-500">{modal.data.id}</p></div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div><span className="text-gray-500">Mobile:</span> <span className="text-white ml-1">{modal.data.mobile}</span></div>
              <div><span className="text-gray-500">Email:</span> <span className="text-white ml-1">{modal.data.email}</span></div>
              <div><span className="text-gray-500">Plan:</span> <span className="text-white ml-1">{modal.data.plan}</span></div>
              <div><span className="text-gray-500">Status:</span> <span className="ml-1">{statusBadge(modal.data.status)}</span></div>
              <div><span className="text-gray-500">Joined:</span> <span className="text-white ml-1">{modal.data.joinedDate}</span></div>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={modal?.type === 'edit'} onClose={() => setModal(null)} title={`Edit: ${modal?.data?.name || ''}`} size="md">
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <div className="space-y-1.5"><label className="text-[10px] text-gray-500">Plan</label><select defaultValue={modal?.data?.plan} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-white focus:outline-none focus:border-ydl-yellow/40"><option>Annual Gold</option><option>Monthly Basic</option><option>Quarterly Pro</option><option>Monthly Pro</option><option>Annual Platinum</option></select></div>
      ...
      <Modal open={modal?.type === 'delete'} onClose={() => setModal(null)} title="Confirm Delete" size="sm">
        <p className="text-xs text-gray-400">Are you sure you want to delete <span className="text-white">{modal?.data?.name}</span>?</p>
        <div className="flex items-center gap-3 mt-4">
          <button onClick={() => { setModal(null) }} className="px-4 py-2 text-xs font-semibold text-white bg-red-500 rounded-lg hover:bg-red-600"><Trash2 className="w-3 h-3 inline mr-1" /> Delete</button>
          <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-gray-400 bg-white/5 border border-ydl-dark-border rounded-lg hover:text-white">Cancel</button>
        </div>
      </Modal>
    </div>
