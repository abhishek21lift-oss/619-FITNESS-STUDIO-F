...
export default function AnalysisDailyCollection() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-[#1C1C1E]">Daily Collection</h1>
        <p className="text-xs text-apple-gray-500 mt-0.5">Track daily revenue by payment method.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatsCard label="Total Collection" value={formatRupee(grandTotal)} icon={IndianRupee} color="from-apple-blue/20 to-ydl-yellow/5" border="border-ydl-yellow/30" text="text-apple-blue" index={0} />
        ...
        <StatsCard label="UPI" value={formatRupee(totalUPI)} icon={Smartphone} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" index={2} />
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] p-5">
        <h2 className="text-xs font-semibold text-[#1C1C1E] mb-4">Daily Collection Trend</h2>
        <div className="h-40">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <Table
          columns={[
            { header: 'Date', accessor: r => <span className="text-[#1C1C1E] font-medium">{r.date}</span> },
            ...
            { header: 'Total', accessor: r => <span className="text-apple-blue font-medium">{formatRupee(r.cash + r.upi + r.card + r.bankTransfer)}</span> },
          ]}
          data={dailyData}
