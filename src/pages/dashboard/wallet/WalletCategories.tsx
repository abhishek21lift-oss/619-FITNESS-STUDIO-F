  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Wallet Categories</h1><p className="text-xs text-apple-gray-500 mt-0.5">Manage income and expense categories.</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Table
          columns={[
            { header: 'Category Name', accessor: (r: WalletCategory) => <div className="flex items-center gap-2"><Tags className="w-3.5 h-3.5 text-apple-blue" /><span className="text-[#1C1C1E] font-medium">{r.name}</span></div> },
            ...
            { header: 'Description', accessor: (r: WalletCategory) => <span className="text-apple-gray-400">{r.description}</span> },
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editCategory ? 'Edit Category' : 'Add Category'} size="sm">
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <input value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Monthly membership payments" />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-apple-gray-400">Status</label>
            <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as WalletCategory['status'] }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40">
              <option>Active</option><option>Inactive</option>
            </select>
          </div>
        </div>
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} disabled={!form.name} className="flex-1 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 disabled:opacity-40">{editCategory ? 'Update' : 'Add'} Category</button>
