...
const statusColors: Record<string, string> = {
  ...
  Completed: 'text-[#007AFF] bg-blue-500/10',
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Diet Plans</h1><p className="text-xs text-apple-gray-500 mt-0.5">Manage client diet plans assigned by dietitians.</p></div>
        <button onClick={() => navigate('/dashboard/app-settings/diet-plans/create')} className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">
          <Plus className="w-3.5 h-3.5" /> Add Plan
        </button>
      </div>

      <div className="flex items-center gap-2 bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 max-w-xs">
        <Search className="w-3.5 h-3.5 text-apple-gray-500" />
        <input value={search} onChange={e => setSearch(e.target.value)} className="bg-transparent text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none w-full" placeholder="Search plans or clients..." />
      </div>

      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Table
          columns={[
            { header: 'Plan Name', accessor: (r: DietPlan) => <span className="text-[#1C1C1E] font-medium">{r.planName}</span> },
            { header: 'Client', accessor: (r: DietPlan) => <div className="flex items-center gap-2"><Salad className="w-3 h-3 text-apple-blue" /><span>{r.clientName}</span></div> },
