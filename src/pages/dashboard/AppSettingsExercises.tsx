  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-white">Exercises</h1><p className="text-xs text-gray-500 mt-0.5">Manage the exercise library for workout plans.</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Table
          columns={[
            { header: 'Exercise Name', accessor: (r: Exercise) => <div className="flex items-center gap-2"><Dumbbell className="w-3.5 h-3.5 text-ydl-yellow" /><span className="text-white font-medium">{r.exerciseName}</span></div> },
            { header: 'Category', accessor: (r: Exercise) => <span className="text-ydl-yellow">{r.category}</span> },
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editExercise ? 'Edit Exercise' : 'Add Exercise'} size="md">
        <div className="space-y-3">
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-gray-400">Exercise Name *</label>
            <input value={form.exerciseName} onChange={e => setForm(p => ({ ...p, exerciseName: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Bench Press" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-gray-400">Equipment</label>
              <select value={form.equipment} onChange={e => setForm(p => ({ ...p, equipment: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <select value={form.difficulty} onChange={e => setForm(p => ({ ...p, difficulty: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
                {difficultyOptions.map(d => <option key={d}>{d}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-gray-400">Default Sets</label>
              <input type="number" min={1} value={form.defaultSets} onChange={e => setForm(p => ({ ...p, defaultSets: parseInt(e.target.value) || 3 }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40" />
            </div>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-gray-400">Default Reps</label>
            <input value={form.defaultReps} onChange={e => setForm(p => ({ ...p, defaultReps: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="8-12" />
          </div>
        </div>
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} disabled={!form.exerciseName} className="flex-1 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90 disabled:opacity-40">{editExercise ? 'Update' : 'Add'} Exercise</button>
