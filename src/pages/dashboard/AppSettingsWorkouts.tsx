  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={tab === 'workout' ? openAddWorkout : openAddDiet} className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90"><Plus className="w-3.5 h-3.5" /> Add {tab === 'workout' ? 'Workout' : 'Diet Plan'}</button>
      </div>

      <div className="flex gap-1.5">
        <button onClick={() => setTab('workout')} className={`px-3 py-1.5 text-[10px] font-medium rounded-lg border transition-all ${tab === 'workout' ? 'bg-ydl-yellow/10 border-ydl-yellow/30 text-ydl-yellow' : 'bg-white/5 border-ydl-dark-border text-gray-500'}`}><Dumbbell className="w-3 h-3 inline mr-1" />Workout Plans</button>
        <button onClick={() => setTab('diet')} className={`px-3 py-1.5 text-[10px] font-medium rounded-lg border transition-all ${tab === 'diet' ? 'bg-ydl-yellow/10 border-ydl-yellow/30 text-ydl-yellow' : 'bg-white/5 border-ydl-dark-border text-gray-500'}`}><Salad className="w-3 h-3 inline mr-1" />Diet Plans</button>
      </div>

      {tab === 'workout' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {workouts.map((w, i) => (
            <motion.div key={w.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <Dumbbell className="w-4 h-4 text-ydl-yellow" />
                  <div><p className="text-xs font-medium text-white">{w.name}</p><p className="text-[9px] text-gray-500">{w.duration} · {w.difficulty}</p></div>
                </div>
                <ActionMenu actions={[
              ...
              <p className="text-[9px] text-gray-500 mt-2 line-clamp-2">{w.description}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {diets.map((d, i) => (
            <motion.div key={d.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <Salad className="w-4 h-4 text-ydl-yellow" />
                  <div><p className="text-xs font-medium text-white">{d.name}</p><p className="text-[9px] text-gray-500">{d.duration}</p></div>
                </div>
                <ActionMenu actions={[
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-[9px] text-gray-500"><Flame className="w-2.5 h-2.5 inline mr-0.5" />{d.calories} cal</span>
                <span className="text-[9px] text-gray-500">{d.meals} meals</span>
              </div>
              <p className="text-[9px] text-gray-500 mt-1 line-clamp-2">{d.description}</p>
            </motion.div>
          ))}
        </div>
      )}
        ...
        {editMode === 'workout' ? (
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-gray-400">Plan Name</label>
              <input value={workoutForm.name} onChange={e => setWorkoutForm(p => ({ ...p, name: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Full Body Blast" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] font-medium text-gray-400">Category</label>
                <select value={workoutForm.category} onChange={e => setWorkoutForm(p => ({ ...p, category: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
                  {workoutCategories.map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-medium text-gray-400">Difficulty</label>
                <select value={workoutForm.difficulty} onChange={e => setWorkoutForm(p => ({ ...p, difficulty: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
            ...
            <div className="space-y-1">
              ...
              <input value={workoutForm.duration} onChange={e => setWorkoutForm(p => ({ ...p, duration: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="45 min" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-gray-400">Description</label>
              <textarea value={workoutForm.description} onChange={e => setWorkoutForm(p => ({ ...p, description: e.target.value }))} rows={2} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" />
            </div>
          </div>
        ) : (
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-gray-400">Plan Name</label>
              <input value={dietForm.name} onChange={e => setDietForm(p => ({ ...p, name: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Weight Loss Plan" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] font-medium text-gray-400">Type</label>
                <select value={dietForm.type} onChange={e => setDietForm(p => ({ ...p, type: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
                  {dietTypes.map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-medium text-gray-400">Calories</label>
                <input value={dietForm.calories} onChange={e => setDietForm(p => ({ ...p, calories: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="2,400" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] font-medium text-gray-400">Meals per Day</label>
                <input type="number" value={dietForm.meals} onChange={e => setDietForm(p => ({ ...p, meals: parseInt(e.target.value) || 3 }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-medium text-gray-400">Duration</label>
                <input value={dietForm.duration} onChange={e => setDietForm(p => ({ ...p, duration: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="12 weeks" />
              </div>
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-gray-400">Description</label>
              <textarea value={dietForm.description} onChange={e => setDietForm(p => ({ ...p, description: e.target.value }))} rows={2} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" />
            </div>
          </div>
        )}
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} disabled={editMode === 'workout' ? !workoutForm.name : !dietForm.name} className="flex-1 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90 disabled:opacity-40">{editId ? 'Update' : 'Add'} Plan</button>
