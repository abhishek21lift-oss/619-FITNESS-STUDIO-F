  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-white">Plan Extensions</h1><p className="text-xs text-gray-500 mt-0.5">Membership extension records.</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <StatsCard label="Total Days Added" value={totalDays} icon={Calendar} color="from-blue-500/20 to-blue-600/5" border="border-blue-500/30" text="text-blue-400" />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-ydl-dark-border bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase">Extended To</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase">Days Added</th>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Extension' : 'Add Extension'} size="md">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-gray-400">Days Added</label>
              <input type="number" value={form.daysAdded} onChange={e => setForm(prev => ({ ...prev, daysAdded: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="0" />
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-gray-400">Extended To</label>
            <input type="date" value={form.extendedTo} onChange={e => setForm(prev => ({ ...prev, extendedTo: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white [color-scheme:dark] focus:outline-none focus:border-ydl-yellow/40" />
          </div>
          <div className="flex items-center gap-3 pt-3 border-t border-ydl-dark-border">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90">{editItem ? 'Update' : 'Add Extension'}</button>
