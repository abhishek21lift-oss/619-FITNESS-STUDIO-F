...
export default function WalletRegister() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div><h1 className="text-lg font-bold text-[#1C1C1E]">Wallet Register</h1><p className="text-xs text-apple-gray-500 mt-0.5">Ledger of all wallet transactions with running balance.</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Table
          columns={[
            { header: 'Date', accessor: (r: RegisterRow) => <span className="text-apple-gray-400">{r.date}</span> },
            { header: 'Member', accessor: (r: RegisterRow) => <span className="text-[#1C1C1E] font-medium">{r.member}</span> },
            { header: 'Activity', accessor: (r: RegisterRow) => <span>{r.activity}</span> },
            { header: 'Debit', accessor: (r: RegisterRow) => r.debit > 0 ? <span className="text-red-400 font-medium flex items-center gap-1"><ArrowDownRight className="w-3 h-3" />₹{r.debit.toLocaleString('en-IN')}</span> : <span className="text-apple-gray-400">-</span> },
            { header: 'Credit', accessor: (r: RegisterRow) => r.credit > 0 ? <span className="text-emerald-400 font-medium flex items-center gap-1"><ArrowUpRight className="w-3 h-3" />₹{r.credit.toLocaleString('en-IN')}</span> : <span className="text-apple-gray-400">-</span> },
            { header: 'Balance', accessor: (r: RegisterRow) => <span className="text-apple-blue font-medium">₹{r.balance.toLocaleString('en-IN')}</span> },
          ]}
          data={filtered}
          keyExtractor={r => r.id}
        />
