...
const pieColors = ['#007AFF', '#EC4899', '#3B82F6', '#10B981', '#8B5CF6', '#F59E0B', '#EF4444', '#6B7280']
...
export default function AnalysisLeadSource() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-[#1C1C1E]">Lead Source Analysis</h1>
        <p className="text-xs text-apple-gray-500 mt-0.5">Break down leads by source and track conversion.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatsCard label="Total Leads" value={totalLeads} icon={Users} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" index={0} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] p-5">
          <h2 className="text-xs font-semibold text-[#1C1C1E] mb-4">Lead Distribution</h2>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <h2 className="text-xs font-semibold text-[#1C1C1E] mb-3">Source-wise Table</h2>
          <Table
            columns={[
              { header: 'Source', accessor: r => r.name },
              { header: 'Group', accessor: r => (
                <span className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${
                  r.group === 'Online' ? 'bg-blue-500/10 text-[#007AFF]' : r.group === 'Offline' ? 'bg-yellow-500/10 text-yellow-400' : 'bg-purple-500/10 text-purple-400'
                }`}>{r.group}</span>
              )},
                  ...
                  <div className="w-20 h-6">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={[{ name: '', rate }]} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        <Bar dataKey="rate" fill="#007AFF" radius={[2, 2, 2, 2]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                )
