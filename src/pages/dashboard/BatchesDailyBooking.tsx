...
export default function BatchesDailyBooking() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-white">Daily Bookings</h1>
        <p className="text-xs text-gray-500 mt-0.5">View daily class bookings and payments.</p>
      </div>

      <div className="flex items-center gap-4">
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium text-gray-400">Filter by Date</label>
          <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} className="w-40 bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 [color-scheme:dark]" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
          <p className="text-[10px] text-blue-400">Pending Payment</p>
          <p className="text-xl font-bold text-blue-400 mt-1">{filtered.filter(b => b.paymentStatus === 'Pending').length}</p>
        </div>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-ydl-dark-border">
              ...
              <th className="text-left px-4 py-3 text-[10px] font-medium text-gray-500 uppercase tracking-wider">Payment</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((b, i) => (
              <tr key={b.id} className="border-b border-ydl-dark-border/50 last:border-0 hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-4 py-3 text-xs text-white">{b.memberName}</td>
                <td className="px-4 py-3 text-xs text-gray-300">{b.className}</td>
                ...
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-full ${b.paymentStatus === 'Paid' ? 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' : b.paymentStatus === 'Pending' ? 'text-yellow-400 bg-yellow-500/10 border border-yellow-500/20' : 'text-gray-400 bg-gray-500/10 border border-gray-500/20'}`}>
                    {b.paymentStatus}
                  </span>
                </td>
              </tr>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="px-4 py-8 text-center text-xs text-gray-500">No bookings found for this date.</td></tr>
            )}
          </tbody>
        </table>
      </motion.div>
