  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-white">Wallet Activities</h1><p className="text-xs text-gray-500 mt-0.5">Manage billable activities and services.</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Table
          columns={[
            { header: 'Activity Name', accessor: (r: WalletActivity) => <div className="flex items-center gap-2"><Activity className="w-3.5 h-3.5 text-ydl-yellow" /><span className="text-white font-medium">{r.activityName}</span></div> },
            { header: 'Category', accessor: (r: WalletActivity) => <span className="text-ydl-yellow">{r.category}</span> },
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editActivity ? 'Edit Activity' : 'Add Activity'} size="md">
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as WalletActivity['status'] }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
              <option>Active</option><option>Inactive</option>
            </select>
          </div>
        </div>
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} disabled={!form.activityName} className="flex-1 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90 disabled:opacity-40">{editActivity ? 'Update' : 'Add'} Activity</button>
