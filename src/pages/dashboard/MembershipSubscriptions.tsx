...
export default function MembershipSubscriptions() {
  ...
  const tabColors: Record<TabType, string> = { All: 'text-blue-400 border-blue-400', Active: 'text-emerald-400 border-emerald-400', Expired: 'text-red-400 border-red-400', 'About to Expire': 'text-amber-400 border-amber-400' }
    ...
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-ydl-dark-border">
          <Layers className="w-3.5 h-3.5 text-ydl-yellow" /><span className="text-[10px] font-semibold text-gray-300">{stats.active} Active</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-ydl-dark-border pb-0">
        {tabs.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-3 py-2 text-[10px] font-semibold uppercase tracking-wider border-b-2 transition-colors ${activeTab === tab ? `${tabColors[tab]}` : 'text-gray-500 border-transparent hover:text-gray-300'}`}>{tab}</button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors" placeholder="Search member or ID..." />
        </div>
        <select value={planFilter} onChange={e => setPlanFilter(e.target.value)} className="bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
          {planOptions.map(p => <option key={p}>{p}</option>)}
        </select>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-ydl-dark-border bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase">Start Date</th>
            ...
            <tbody className="divide-y divide-ydl-dark-border/50">
              {paged.map((s, i) => (
                <motion.tr key={s.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-400">{s.startDate}</td>
                  <td className="px-4 py-3 text-xs text-gray-400">{s.endDate}</td>
      ...
      <Modal open={renewModal} onClose={() => setRenewModal(false)} title="Renew Subscription" size="sm">
        <div className="space-y-4">
          <p className="text-xs text-gray-400">Renewing <span className="text-white font-medium">{selectedSub?.member}</span> — <span className="text-ydl-yellow">{selectedSub?.plan}</span></p>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-gray-400">New Start Date</label>
            <input type="date" value={renewDate} onChange={e => setRenewDate(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 [color-scheme:dark]" />
          </div>
          <div className="flex items-center gap-3 pt-3 border-t border-ydl-dark-border">
            <button onClick={handleRenew} className="px-4 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90">Confirm Renewal</button>
            <button onClick={() => setRenewModal(false)} className="px-4 py-2 text-xs font-medium text-gray-400 hover:text-white">Cancel</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={cancelModal} onClose={() => setCancelModal(false)} title="Cancel Subscription" size="sm">
        <div className="space-y-4">
          <p className="text-xs text-gray-400">Cancelling <span className="text-white font-medium">{selectedSub?.member}</span> — <span className="text-ydl-yellow">{selectedSub?.plan}</span></p>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-gray-400">Reason for Cancellation</label>
            <select value={cancelReason} onChange={e => setCancelReason(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-ydl-dark-border">
            ...
            <button onClick={() => setCancelModal(false)} className="px-4 py-2 text-xs font-medium text-gray-400 hover:text-white">Keep Subscription</button>
          </div>
        </div>
      </Modal>
        ...
        {profileMember && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><p className="text-[10px] text-gray-500">Plan</p><p className="text-xs text-white">{profileMember.plan}</p></div>
              <div><p className="text-[10px] text-gray-500">Amount</p><p className="text-xs text-ydl-yellow">{profileMember.amount}</p></div>
              <div><p className="text-[10px] text-gray-500">Start Date</p><p className="text-xs text-white">{profileMember.startDate}</p></div>
              <div><p className="text-[10px] text-gray-500">End Date</p><p className="text-xs text-white">{profileMember.endDate}</p></div>
            </div>
            <div className="pt-2">
