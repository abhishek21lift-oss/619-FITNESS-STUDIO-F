...
const initialPlatforms: SocialPlatform[] = [
  ...
  { id: 'linkedin', name: 'LinkedIn', icon: LinkedinIcon, color: 'text-blue-400', bg: 'bg-blue-500/10', url: 'https://linkedin.com/company/yourdigitallift', enabled: false },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-5">
          ...
          <div className="space-y-3">
            {platforms.map(p => (
              <div key={p.id} className="p-3 rounded-lg bg-white/[0.03] border border-ydl-dark-border">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => testLink(p.url)} className="p-1 text-gray-500 hover:text-ydl-yellow" title="Test link"><ExternalLink className="w-3 h-3" /></button>
                ...
                <input value={p.url} onChange={e => updateUrl(p.id, e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-2.5 py-1.5 text-[10px] text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="https://..." />
              </div>
            ))}
          </div>
          <button onClick={handleSave} className="mt-4 w-full py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90"><Save className="w-3.5 h-3.5 inline mr-1" /> {saved ? 'Saved!' : 'Save Links'}</button>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-5">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="w-full space-y-2 mb-4">
              <label className="text-[10px] font-medium text-gray-400">QR Code Link</label>
              <input value={qrLink} onChange={e => setQrLink(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" />
            </div>
            <p className="text-[10px] text-gray-500 text-center">Scan to visit your page</p>
            <div className="flex items-center gap-2 mt-3">
              <button onClick={() => testLink(qrLink)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90"><QrCode className="w-3.5 h-3.5" /> Download QR</button>
              <button onClick={() => setQrLink('https://yourdigitallift.com')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-400 bg-white/5 border border-ydl-dark-border rounded-lg hover:text-white"><RefreshCw className="w-3.5 h-3.5" /> Regenerate</button>
            </div>
          </div>
        </motion.div>
      </div>
