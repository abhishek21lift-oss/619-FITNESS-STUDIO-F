  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">E-commerce</h1><p className="text-xs text-apple-gray-500 mt-0.5">Manage gym merchandise and products.</p></div>
        <button onClick={openAdd} className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90"><Plus className="w-3.5 h-3.5" /> Add Product</button>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <StatsCard label="Total Products" value={products.length} icon={Package} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {paged.map((p, i) => (
          <motion.div key={p.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] overflow-hidden">
            <div className="aspect-square bg-gradient-to-br from-apple-blue/10 to-amber-600/5 flex items-center justify-center relative">
              <Package className="w-12 h-12 text-apple-gray-400" />
              {p.stock <= 10 && p.status === 'Active' && (
                <span className="absolute top-2 right-2 px-1.5 py-0.5 text-[8px] font-bold text-[#1C1C1E] bg-red-500/80 rounded-md flex items-center gap-0.5">
            ...
            <div className="p-3">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <h3 className="text-xs font-semibold text-[#1C1C1E] truncate">{p.name}</h3>
                  <span className="text-[9px] text-apple-gray-500">{p.category}</span>
                </div>
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-md ${p.status === 'Active' ? 'text-emerald-400 bg-emerald-500/10' : 'text-apple-gray-400 bg-gray-500/10'}`}>{p.status}</span>
              </div>
              <p className="text-[9px] text-apple-gray-500 mt-1 line-clamp-2">{p.description}</p>
              <div className="flex items-center justify-between mt-2">
                <span className="text-sm font-bold text-apple-blue">{p.price}</span>
                <span className={`text-[9px] font-medium ${p.stock <= 10 ? 'text-red-400' : 'text-apple-gray-400'}`}>Stock: {p.stock}</span>
              </div>
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-apple-gray-200">
                <ActionMenu actions={[
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as Product['status'] }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <input value={form.price} onChange={e => setForm(p => ({ ...p, price: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="₹799" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-medium text-apple-gray-400">Stock Count</label>
              <input type="number" value={form.stock} onChange={e => setForm(p => ({ ...p, stock: parseInt(e.target.value) || 0 }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40" />
            </div>
          </div>
          <div className="space-y-1">
          ...
          <div className="border-2 border-dashed border-apple-gray-200 rounded-lg p-4 text-center hover:border-ydl-yellow/30 transition-colors cursor-pointer">
            <Package className="w-6 h-6 text-apple-gray-400 mx-auto mb-1" />
            <p className="text-[10px] text-apple-gray-500">Product image (optional)</p>
          </div>
        </div>
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} disabled={!form.name} className="flex-1 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 disabled:opacity-40">{editProduct ? 'Update' : 'Add'} Product</button>
