  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Receipt Log</h1><p className="text-xs text-apple-gray-500 mt-0.5">Audit trail for receipt actions.</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatsCard label="Total Logs" value={data.length} icon={ClipboardList} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" />
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg pl-9 pr-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Search logs..." />
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-apple-gray-200 bg-white/[0.03]">
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase">Log#</th>
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase">Timestamp</th>
            ...
            <tbody className="divide-y divide-apple-gray-200/50">
              {filtered.map((d, i) => (
                <motion.tr key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3 text-xs font-mono text-apple-blue">{d.logNo}</td>
                  <td className="px-4 py-3 text-xs text-apple-gray-400">{d.date}</td>
                  <td className="px-4 py-3 text-xs font-mono text-apple-gray-400">{d.receiptNo}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded-md border ${
                      ...
                      'text-[#007AFF] bg-blue-500/10 border-blue-500/20'
                    }`}>{d.action}</span>
                  </td>
                  <td className="px-4 py-3 text-xs text-apple-gray-400">{d.staff}</td>
                  <td className="px-4 py-3 text-xs text-apple-gray-400">{d.timestamp}</td>
