...
export default function AnalysisSalesLeaderboard() {
  ...
  const getRankBadge = (rank: number) => {
    ...
    if (rank === 2) return <Trophy className="w-4 h-4 text-apple-gray-400" />
    if (rank === 3) return <Trophy className="w-4 h-4 text-amber-600" />
    return <span className="text-[10px] text-apple-gray-500 w-4 text-center inline-block">{rank}</span>
  }

  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-[#1C1C1E]">Sales Leaderboard</h1>
        <p className="text-xs text-apple-gray-500 mt-0.5">Staff rankings by sales performance.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <StatsCard label="Avg Achievement" value={`${avgAchievement}%`} icon={Target} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" index={2} />
      </div>

      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-xs font-semibold text-[#1C1C1E] mb-3">Staff Rankings</h2>
        <Table
          columns={[
