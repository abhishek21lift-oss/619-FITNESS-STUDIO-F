  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        <StatsCard label="Total Feedback" value={feedbacks.length} icon={MessageSquare} color="from-blue-500/20 to-blue-600/5" border="border-blue-500/30" text="text-blue-400" />
        <StatsCard label="Avg Rating" value={avgRating} icon={Star} color="from-ydl-yellow/20 to-ydl-yellow/5" border="border-ydl-yellow/30" text="text-ydl-yellow" />
      ...
      <div className="flex gap-1.5">
        {(['All', 'New', 'Read', 'Resolved'] as const).map(t => (
          <button key={t} onClick={() => setStatusTab(t)} className={`px-3 py-1.5 text-[10px] font-medium rounded-lg border transition-all ${statusTab === t ? 'bg-ydl-yellow/10 border-ydl-yellow/30 text-ydl-yellow' : 'bg-white/5 border-ydl-dark-border text-gray-500 hover:text-gray-300'}`}>
            {t} ({statusCounts[t]})
          </button>
        ))}
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
        {filtered.map((f, i) => (
          <motion.div
            ...
            className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-4 cursor-pointer hover:bg-white/[0.04] transition-colors"
            onClick={() => setSelectedFeedback(f)}
          >
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-white">{f.member}</span>
                  <div className="flex items-center gap-0.5">
                    {Array.from({ length: 5 }).map((_, si) => <Star key={si} className={`w-3 h-3 ${si < f.rating ? 'text-ydl-yellow' : 'text-gray-600'}`} />)
                  }</div>
                  <span className={`inline-flex items-center px-1.5 py-0.5 text-[8px] font-medium rounded-md ${
                    f.status === 'New' ? 'text-blue-400 bg-blue-500/10' : f.status === 'Read' ? 'text-amber-400 bg-amber-500/10' : 'text-emerald-400 bg-emerald-500/10'
                  }`}>{f.status}</span>
                </div>
                <p className="text-[11px] text-gray-400 mt-1 line-clamp-2">{f.comment}</p>
              </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <div className="flex items-center gap-0.5">
                  {Array.from({ length: 5 }).map((_, si) => <Star key={si} className={`w-4 h-4 ${si < selectedFeedback.rating ? 'text-ydl-yellow' : 'text-gray-600'}`} />)
                }</div>
                <span className={`inline-flex items-center px-1.5 py-0.5 text-[9px] font-medium rounded-md ${
                  selectedFeedback.status === 'New' ? 'text-blue-400 bg-blue-500/10' : selectedFeedback.status === 'Read' ? 'text-amber-400 bg-amber-500/10' : 'text-emerald-400 bg-emerald-500/10'
                }`}>{selectedFeedback.status}</span>
              </div>
              <p className="text-xs text-gray-300">{selectedFeedback.comment}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="text-[9px] text-gray-600">{selectedFeedback.date}</span>
                <span className="text-[10px] font-medium text-ydl-yellow bg-ydl-yellow/10 px-2 py-0.5 rounded-md">{selectedFeedback.category}</span>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-ydl-dark-border">
              <h4 className="text-[11px] font-semibold text-white mb-2">Reply</h4>
              {replies[selectedFeedback.id]?.map((r, ri) => (
                <div key={ri} className="p-2 rounded-lg bg-ydl-yellow/5 border border-ydl-yellow/10 mb-2">
                  ...
                  <p className="text-[8px] text-gray-600 mt-0.5">Admin · Just now</p>
                </div>
              ))}
              <div className="flex items-center gap-2">
                <textarea value={replyText} onChange={e => setReplyText(e.target.value)} rows={2} className="flex-1 bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" placeholder="Type your reply..." />
                <button onClick={handleReply} disabled={!replyText.trim()} className="p-2 text-black bg-ydl-gradient rounded-lg hover:opacity-90 disabled:opacity-40"><Send className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        )}
