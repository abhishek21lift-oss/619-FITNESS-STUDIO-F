  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-[#1C1C1E]">Members Analysis</h1>
          <p className="text-xs text-apple-gray-500 mt-0.5">Member growth, retention, and composition.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <StatsCard label="Total Active" value={totalActive} icon={Users} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" index={1} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] p-5">
          <h2 className="text-xs font-semibold text-[#1C1C1E] mb-4">New vs Churned</h2>
          <div className="h-44">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] p-5">
          <h2 className="text-xs font-semibold text-[#1C1C1E] mb-4">Member Type Breakdown</h2>
          <Table
            columns={[
              { header: 'Type', accessor: r => r.type },
              { header: 'Count', accessor: r => <span className="text-[#1C1C1E] font-medium">{r.count}</span> },
              { header: '%', accessor: r => r.pct + '%' },
              { header: 'Bar', accessor: r => (
                <div className="w-24 h-2 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-apple-blue/60 to-ydl-yellow/30" style={{ width: `${r.pct * 1.6}%` }} />
                </div>
              )},
            ]}
            data={memberTypes}
      ...
      <Modal open={exportOpen} onClose={() => setExportOpen(false)} title="Export Members Report" size="sm">
        <p className="text-xs text-apple-gray-400 mb-3">Download member analysis as CSV:</p>
