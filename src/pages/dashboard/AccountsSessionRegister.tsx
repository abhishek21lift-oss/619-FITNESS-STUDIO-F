...
import { DollarSign, Calendar, CheckCircle, Plus, Search, Download } from 'lucide-react'
...
const initialData: Session[] = [
  { id: 1, date: '07 Jun 2026', member: 'Priya Singh', trainer: 'Amit', sessionNo: 12, amount: 1500, status: 'Completed' },
  { id: 2, date: '07 Jun 2026', member: 'Rahul Sharma', trainer: 'Neha', sessionNo: 8, amount: 1500, status: 'Completed' },
  { id: 3, date: '06 Jun 2026', member: 'Sneha Patel', trainer: 'Ravi', sessionNo: 5, amount: 2000, status: 'Pending' },
  { id: 4, date: '05 Jun 2026', member: 'Vikram Joshi', trainer: 'Amit', sessionNo: 15, amount: 1500, status: 'Cancelled' },
  { id: 5, date: '04 Jun 2026', member: 'Anita Desai', trainer: 'Neha', sessionNo: 3, amount: 1500, status: 'Completed' },
]

export default function AccountsSessionRegister() {
  ...
  const [form, setForm] = useState({ date: '', member: '', trainer: '', sessionNo: '', amount: '', status: 'Pending' as Session['status'] })
  ...
  const openAdd = () => {
    setEditItem(null)
    setForm({ date: new Date().toISOString().split('T')[0], member: '', trainer: '', sessionNo: '', amount: '', status: 'Pending' })
    setModalOpen(true)
  }

  const openEdit = (item: Session) => {
    setEditItem(item)
    setForm({ date: item.date, member: item.member, trainer: item.trainer, sessionNo: String(item.sessionNo), amount: String(item.amount), status: item.status })
    setModalOpen(true)
  }
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      setData(prev => prev.map(d => d.id === editItem.id ? { ...d, date: form.date, member: form.member, trainer: form.trainer, sessionNo: Number(form.sessionNo), amount: Number(form.amount), status: form.status } : d))
    } else {
      setData(prev => [...prev, { id: Math.max(...prev.map(d => d.id), 0) + 1, date: form.date, member: form.member, trainer: form.trainer, sessionNo: Number(form.sessionNo), amount: Number(form.amount), status: form.status }])
    }
    setModalOpen(false)
  }
  ...
  const exportCSV = () => {
    const headers = ['Date', 'Member', 'Trainer', 'Session#', 'Amount', 'Status']
    const rows = filtered.map(d => [d.date, d.member, d.trainer, String(d.sessionNo), `₹${d.amount}`, d.status])
    const csv = [headers.join(','), ...rows.map(r => r.map(v => `"${v}"`).join(','))].join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `session-register-${new Date().toISOString().split('T')[0]}.csv`
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Session Register</h1><p className="text-xs text-apple-gray-500 mt-0.5">Paid session records.</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatsCard label="Session Revenue" value={fmt(totalRevenue)} icon={DollarSign} color="from-emerald-500/20 to-emerald-600/5" border="border-emerald-500/30" text="text-emerald-400" />
        <StatsCard label="Completed Sessions" value={completedCount} icon={CheckCircle} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" />
        <StatsCard label="Total Sessions" value={data.length} icon={Calendar} color="from-purple-500/20 to-purple-600/5" border="border-purple-500/30" text="text-purple-400" />
      </div>

      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-apple-gray-500" />
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg pl-9 pr-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Search sessions..." />
        </div>
        <div className="flex items-center gap-2">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-apple-gray-200 bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase">Session#</th>
            ...
            <tbody className="divide-y divide-apple-gray-200/50">
              {filtered.map((d, i) => (
                <motion.tr key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-apple-gray-400">#{d.sessionNo}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Session' : 'Add Session'} size="md">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <input value={form.trainer} onChange={e => setForm(prev => ({ ...prev, trainer: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Trainer name..." />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-apple-gray-400">Session#</label>
              <input type="number" value={form.sessionNo} onChange={e => setForm(prev => ({ ...prev, sessionNo: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="0" />
            </div>
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-apple-gray-400">Status</label>
              <select value={form.status} onChange={e => setForm(prev => ({ ...prev, status: e.target.value as Session['status'] }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-apple-gray-200">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">{editItem ? 'Update' : 'Add Session'}</button>
