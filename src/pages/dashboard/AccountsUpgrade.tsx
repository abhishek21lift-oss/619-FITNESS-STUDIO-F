...
import { TrendingUp, DollarSign, Plus, Search, Download } from 'lucide-react'
...
const initialData: Upgrade[] = [
  { id: 1, date: '07 Jun 2026', member: 'Rahul Sharma', fromPlan: 'Quarterly Basic', toPlan: 'Annual Gold', diffAmount: 4499, staff: 'Amit' },
  { id: 2, date: '06 Jun 2026', member: 'Priya Singh', fromPlan: 'Monthly Basic', toPlan: 'Monthly Premium', diffAmount: 1000, staff: 'Neha' },
  { id: 3, date: '05 Jun 2026', member: 'Sneha Patel', fromPlan: 'Annual Gold', toPlan: 'Annual Platinum', diffAmount: 7000, staff: 'Ravi' },
  { id: 4, date: '04 Jun 2026', member: 'Amit Verma', fromPlan: 'Monthly Premium', toPlan: 'Quarterly Premium', diffAmount: 3501, staff: 'Amit' },
  { id: 5, date: '03 Jun 2026', member: 'Deepak Yadav', fromPlan: 'Basic Trial', toPlan: 'Monthly Basic', diffAmount: 999, staff: 'Neha' },
]

export default function AccountsUpgrade() {
  const [data, setData] = useState<Upgrade[]>(initialData)
  const [search, setSearch] = useState('')
  const [dateFrom, setDateFrom] = useState('')
  const [dateTo, setDateTo] = useState('')
  const [modalOpen, setModalOpen] = useState(false)
  const [editItem, setEditItem] = useState<Upgrade | null>(null)
  ...
  const totalUpgradeRevenue = data.reduce((s, d) => s + d.diffAmount, 0)
  ...
  const openEdit = (item: Upgrade) => {
    setEditItem(item)
    setForm({ date: item.date, member: item.member, fromPlan: item.fromPlan, toPlan: item.toPlan, diffAmount: String(item.diffAmount), staff: item.staff })
    setModalOpen(true)
  }
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      setData(prev => prev.map(d => d.id === editItem.id ? { ...d, ...form, diffAmount: Number(form.diffAmount) } : d))
    } else {
      setData(prev => [...prev, { id: Math.max(...prev.map(d => d.id), 0) + 1, date: form.date, member: form.member, fromPlan: form.fromPlan, toPlan: form.toPlan, diffAmount: Number(form.diffAmount), staff: form.staff }])
    }
    setModalOpen(false)
  }
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map(d => [d.date, d.member, d.fromPlan, d.toPlan, `₹${d.diffAmount}`, d.staff])
    const csv = [headers.join(','), ...rows.map(r => r.map(v => `"${v}"`).join(','))].join('\n')
    const blob = new Blob([csv], { type: 'text/csv' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `upgrades-${new Date().toISOString().split('T')[0]}.csv`
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Plan Upgrades</h1><p className="text-xs text-apple-gray-500 mt-0.5">Members who upgraded their plans.</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <StatsCard label="Upgrades" value={data.length} icon={TrendingUp} color="from-emerald-500/20 to-emerald-600/5" border="border-emerald-500/30" text="text-emerald-400" />
        <StatsCard label="Additional Revenue" value={fmt(totalUpgradeRevenue)} icon={DollarSign} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" />
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-apple-gray-200 bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase">Diff Amount</th>
            ...
            <tbody className="divide-y divide-apple-gray-200/50">
              {filtered.map((d, i) => (
                <motion.tr key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs font-semibold text-emerald-400">+{fmt(d.diffAmount)}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Upgrade' : 'Add Upgrade'} size="md">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-apple-gray-400">Diff Amount (₹)</label>
              <input type="number" value={form.diffAmount} onChange={e => setForm(prev => ({ ...prev, diffAmount: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="0" />
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-apple-gray-200">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">{editItem ? 'Update' : 'Add Upgrade'}</button>
