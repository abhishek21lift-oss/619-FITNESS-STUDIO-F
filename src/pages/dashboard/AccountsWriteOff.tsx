...
import { DollarSign, AlertTriangle, XCircle, Plus, Search, Download } from 'lucide-react'
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-white">Write Offs</h1><p className="text-xs text-gray-500 mt-0.5">Amounts written off for adjustments.</p></div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Search write-offs..." />
        </div>
        <div className="flex items-center gap-2">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-ydl-dark-border bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-500 uppercase">Approved By</th>
            ...
            <tbody className="divide-y divide-ydl-dark-border/50">
              {filtered.map((d, i) => (
                <motion.tr key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-400 max-w-[200px] truncate">{d.reason}</td>
                  <td className="px-4 py-3 text-xs text-gray-400">{d.approvedBy}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Write Off' : 'Add Write Off'} size="md">
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={form.reason} onChange={e => setForm(prev => ({ ...prev, reason: e.target.value }))} rows={2} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" placeholder="Write-off reason..." />
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-gray-400">Approved By</label>
            <input value={form.approvedBy} onChange={e => setForm(prev => ({ ...prev, approvedBy: e.target.value }))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg px-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" />
          </div>
          <div className="flex items-center gap-3 pt-3 border-t border-ydl-dark-border">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90">{editItem ? 'Update' : 'Add Write Off'}</button>
