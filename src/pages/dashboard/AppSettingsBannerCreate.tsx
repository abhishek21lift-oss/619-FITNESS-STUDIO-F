    ...
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/dashboard/app-settings/banner')} className="p-2 rounded-lg border border-apple-gray-200 bg-white/5 hover:bg-white/10 transition-colors">
            <ArrowLeft className="w-4 h-4 text-apple-gray-400" />
          </button>
          <div><h1 className="text-lg font-bold text-[#1C1C1E]">Create Banner</h1><p className="text-xs text-apple-gray-500 mt-0.5">Add a new promotional banner.</p></div>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl rounded-xl border border-apple-gray-200 bg-white/[0.02] p-6 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <label className="border-2 border-dashed border-apple-gray-200 rounded-lg p-8 text-center hover:border-ydl-yellow/30 transition-colors cursor-pointer block">
            ...
            <p className="text-[9px] text-apple-gray-400 mt-1">Recommended: 1200x400px</p>
        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <button onClick={() => navigate('/dashboard/app-settings/banner')} className="flex-1 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]"><ArrowLeft className="w-3.5 h-3.5 inline mr-1" />Back</button>
        </div>
      </motion.div>
    </div>
  )
