  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Deleted Subscriptions</h1><p className="text-xs text-apple-gray-500 mt-0.5">Removed subscription records.</p></div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-apple-gray-200 bg-white/[0.03]">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase">Deleted By</th>
            ...
            <tbody className="divide-y divide-apple-gray-200/50">
              {filtered.map((d, i) => (
                <motion.tr key={d.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-apple-gray-400">{d.deletedBy}</td>
                  <td className="px-4 py-3 text-xs text-apple-gray-500 max-w-[200px] truncate">{d.reason}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Deleted Subscription" size="md">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-apple-gray-400">Deleted By</label>
              <input value={form.deletedBy} onChange={e => setForm(prev => ({ ...prev, deletedBy: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-apple-gray-400">Reason</label>
            <textarea value={form.reason} onChange={e => setForm(prev => ({ ...prev, reason: e.target.value }))} rows={2} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" />
          </div>
