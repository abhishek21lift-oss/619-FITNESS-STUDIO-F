    ...
    <div className="p-6 space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {revenue && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-ydl-dark-border bg-ydl-card-gradient p-5">
              <h2 className="text-lg font-bold text-white mb-4">Revenue</h2>
              <p className="text-3xl font-bold text-ydl-yellow mb-4">₹{(revenue.total || 0).toLocaleString()}</p>
              <div className="space-y-2">
                {(revenue.monthly || []).slice(-6).map((m: any) => (
                  <div key={m.month} className="flex items-center justify-between text-sm">
                    <span className="text-ydl-muted">{m.month}</span>
          ...
          {membership && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-2xl border border-ydl-dark-border bg-ydl-card-gradient p-5">
              <h2 className="text-lg font-bold text-white mb-4">Membership Growth</h2>
              <div className="space-y-2">
                {(membership.growth || []).slice(-6).map((g: any) => (
                  <div key={g.month} className="flex items-center justify-between text-sm">
                    <span className="text-ydl-muted">{g.month}</span>
                    <div className="flex-1 mx-3 h-2 bg-ydl-dark-border rounded-full overflow-hidden">
              ...
              <h3 className="text-white font-bold mt-6 mb-3">By Plan</h3>
              <div className="space-y-2">
                {(membership.byPlan || []).map((p: any) => (
                  <div key={p.plan} className="flex items-center justify-between text-sm">
                    <span className="text-ydl-muted">{p.plan}</span>
                    <span className="text-white font-medium">{p.count} members</span>
                  </div>
                ))}
              </div>
            </motion.div>
