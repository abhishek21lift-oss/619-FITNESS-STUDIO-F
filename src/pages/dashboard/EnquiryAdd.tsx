...
import { User, Mail, Phone, Calendar, Users, Tag, MessageSquare, Save } from 'lucide-react'

const sourceOptions = ['Walk-in', 'Instagram', 'Facebook', 'Google', 'Friend Referral', 'Phone Call', 'Website']
const genderOptions = ['Male', 'Female', 'Other']
const followUpTypeOptions = ['Call', 'Visit', 'WhatsApp', 'Email', 'SMS']
const assignTrainerOptions = ['Unassigned', 'Awash Vikash', 'Riya Singh', 'Abhishek Katiyar', 'Rajat Katiyar', 'Narayan Chandel', 'Shivani Verma']
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-5 max-w-2xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2 space-y-1.5">
            ...
            <div className="relative">
              ...
              <input className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors" placeholder="Client name" />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-gray-400">Source</label>
            <div className="relative">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Users className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500" />
              <select className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 transition-colors appearance-none">
                {assignTrainerOptions.map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-gray-400">Follow-up Type</label>
            <div className="relative">
        ...
        <div className="flex items-center gap-3 mt-6 pt-4 border-t border-ydl-dark-border">
          <button className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-black bg-ydl-gradient rounded-lg hover:opacity-90 transition-opacity">
            <Save className="w-3.5 h-3.5" /> Save Enquiry
          </button>
          ...
          <button className="flex items-center gap-2 px-4 py-2 text-xs font-medium text-gray-400 hover:text-white transition-colors">
            Cancel
          </button>
        </div>
      </motion.div>
