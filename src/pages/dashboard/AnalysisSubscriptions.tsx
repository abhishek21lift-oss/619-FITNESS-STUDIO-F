...
export default function AnalysisSubscriptions() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-white">Subscription Analysis</h1>
        <p className="text-xs text-gray-500 mt-0.5">Track subscriptions, renewals, and plan performance.</p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-xs font-semibold text-white mb-3">Plan-wise Breakdown</h2>
        <Table
          columns={[
            ...
            { header: 'Revenue', accessor: r => <span className="text-white font-medium">₹{r.revenue.toLocaleString()}</span> },
            { header: '', accessor: () => (
              <button onClick={() => { setDetailStatus('Active'); setDetailModal(true) }} className="text-ydl-yellow/70 hover:text-ydl-yellow transition-colors">
        ...
        <div className="flex gap-2 mb-3">
          {(['Active', 'Expired', 'About to Expire'] as const).map(s => (
            <button key={s} onClick={() => setDetailStatus(s)} className={`px-2.5 py-1 text-[10px] font-medium rounded-lg border transition-all ${
              detailStatus === s ? 'bg-ydl-yellow/10 border-ydl-yellow/30 text-ydl-yellow' : 'bg-white/5 border-ydl-dark-border text-gray-500'
            }`}>{s}</button>
          ))}
        </div>
        <Table
