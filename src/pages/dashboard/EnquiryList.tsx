...
export default function EnquiryList() {
  ...
  const [enquiries, setEnquiries] = useState<typeof mockEnquiries>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await enquiriesApi.list({ limit: '1000' })
  ...
  const tappedCount = enquiries.filter(e => e.tapped === 'Tapped').length
  const untappedCount = enquiries.filter(e => e.tapped === 'Untapped').length
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <LeadStatusTab label="All" count={enquiries.length} active={leadTab === 'All'} onClick={() => { setLeadTab('All'); setPage(1) }} />
      </div>
