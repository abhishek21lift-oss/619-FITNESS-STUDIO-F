  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Combo Offers</h1><p className="text-xs text-apple-gray-500 mt-0.5">Bundle deals and special packages.</p></div>
        <button onClick={openAdd} className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 transition-opacity"><Plus className="w-3.5 h-3.5" /> Add Combo</button>
      </div>
        ...
        {combos.map((c, i) => (
          <motion.div key={c.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className={`rounded-xl border ${c.active ? 'border-apple-gray-200' : 'border-red-500/20'} bg-white/[0.02] p-4`}>
            <div className="flex items-start justify-between">
              <h3 className="text-sm font-bold text-[#1C1C1E]">{c.name}</h3>
              {c.save && (
            ...
            <div className="mt-2">
              <span className="text-xl font-bold text-apple-blue">{c.price}</span>
              {c.original && <span className="text-xs text-apple-gray-500 line-through ml-2">{c.original}</span>}
            </div>
            <div className="mt-3 space-y-1">
              <p className="text-[10px] font-medium text-apple-gray-500">Plans Included:</p>
              {c.plansIncluded.map(p => (
                <div key={p} className="flex items-center gap-1.5 text-[10px] text-apple-gray-400">
                  <Check className="w-3 h-3 text-apple-blue" />{p}
                </div>
              ))}
            </div>
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-apple-gray-200">
              <span className={`inline-flex items-center px-2 py-0.5 text-[10px] font-medium rounded-full ${c.active ? 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' : 'text-apple-gray-400 bg-gray-500/10 border border-gray-500/20'}`}>{c.active ? 'Active' : 'Inactive'}</span>
              <ActionMenu
                label="Manage"
                actions={[
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editCombo ? 'Edit Combo' : 'Add Combo'} size="md">
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input value={form.name} onChange={e => setForm(prev => ({ ...prev, name: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="e.g. Duo Membership" />
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-apple-gray-400">Combo Price</label>
            <input value={form.price} onChange={e => setForm(prev => ({ ...prev, price: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="e.g. ₹1,499" />
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-apple-gray-400">Select Plans</label>
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-apple-gray-200">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 transition-opacity">{editCombo ? 'Update Combo' : 'Create Combo'}</button>
