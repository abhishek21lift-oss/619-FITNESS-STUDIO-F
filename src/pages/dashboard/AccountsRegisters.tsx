  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Account Registers</h1><p className="text-xs text-apple-gray-500 mt-0.5">General ledger and accounting entries.</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <StatsCard label="Balance" value={fmt(balance)} icon={DollarSign} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-apple-gray-200 bg-white/[0.02] p-0.5">
          {(['All', 'Income', 'Expense'] as const).map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} className={`px-3 py-1 text-[10px] font-medium rounded-md transition-colors ${typeFilter === t ? 'text-black bg-apple-gradient-blue' : 'text-apple-gray-400 hover:text-[#1C1C1E]'}`}>{t}</button>
          ))}
        </div>
      </div>
            ...
            <tbody className="divide-y divide-apple-gray-200/50">
              {filtered.map((e, i) => (
                <motion.tr key={e.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-[#1C1C1E]">{e.description}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editEntry ? 'Edit Entry' : 'Add Entry'} size="md">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 pt-3 border-t border-apple-gray-200">
            <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">{editEntry ? 'Update Entry' : 'Add Entry'}</button>
