  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-[#1C1C1E]">Program Attendance</h1>
          <p className="text-xs text-apple-gray-500 mt-0.5">Mark attendance for batch programs.</p>
        </div>
        <button onClick={markAllPresent} className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 transition-opacity">
      ...
      <div className="flex items-center gap-4">
        <div className="space-y-1.5">
          <label className="text-[11px] font-medium text-apple-gray-400">Program</label>
          <select value={selectedProgram} onChange={e => setSelectedProgram(e.target.value)} className="w-48 bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-apple-gray-200">
              ...
              <th className="text-left px-4 py-3 text-[10px] font-medium text-apple-gray-500 uppercase tracking-wider">Notes</th>
            </tr>
          </thead>
          <tbody>
            {members.map((m, i) => (
              <tr key={m.id} className="border-b border-apple-gray-200/50 last:border-0 hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3 text-xs text-apple-gray-500">{i + 1}</td>
                <td className="px-4 py-3 text-xs text-[#1C1C1E]">{m.name}</td>
                ...
                <td className="px-4 py-3">
                  <input value={m.notes} onChange={e => setNotes(m.id, e.target.value)} className="w-40 bg-white/5 border border-apple-gray-200 rounded px-2 py-1 text-[10px] text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Add notes..." />
                </td>
              </tr>
            ))}
          </tbody>
