  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Business Rules</h1><p className="text-xs text-apple-gray-500 mt-0.5">Configure automated business logic and workflows.</p></div>
      ...
      <div className="relative max-w-xs">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg pl-9 pr-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Search rules..." />
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-apple-gray-200 bg-white/[0.03]">
              <th className="text-left px-4 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase">Rule Name</th>
          ...
          <tbody className="divide-y divide-apple-gray-200/50">
            {filtered.map((r, i) => (
              <motion.tr key={r.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/[0.02] transition-colors">
                <td className="px-4 py-3 text-xs font-medium text-[#1C1C1E]">{r.name}</td>
                <td className="px-4 py-3 text-xs text-apple-gray-400 max-w-[300px] truncate">{r.description}</td>
                <td className="px-4 py-3">
                  <button onClick={() => toggleStatus(r.id)} className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-medium rounded-md border transition-colors ${r.status === 'Active' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-apple-gray-400 bg-white/[0.03] border-apple-gray-200'}`}>
                    <ToggleLeft className={`w-3 h-3 ${r.status === 'Active' ? 'text-emerald-400' : 'text-apple-gray-500'}`} />
                    {r.status}
                  </button>
                </td>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title={editRule ? 'Edit Rule' : 'Add Rule'} size="sm">
        <div className="space-y-3">
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-apple-gray-400">Rule Name</label>
            <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Auto-freeze after 30 days" />
          </div>
          <div className="space-y-1">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2">
              {(['Active', 'Inactive'] as const).map(s => (
                <label key={s} className={`flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-medium rounded-lg cursor-pointer border transition-colors ${form.status === s ? 'text-apple-blue bg-apple-blue/10 border-ydl-yellow/30' : 'text-apple-gray-400 bg-white/5 border-apple-gray-200 hover:text-[#1C1C1E]'}`}>
        ...
        <div className="flex items-center gap-2 mt-4">
          <button onClick={handleSave} disabled={!form.name} className="flex-1 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 disabled:opacity-40">{editRule ? 'Update Rule' : 'Add Rule'}</button>
