...
const pieColors = ['#007AFF', '#EC4899', '#3B82F6', '#10B981', '#8B5CF6', '#6B7280']
...
export default function AnalysisConversion() {
  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div>
        <h1 className="text-lg font-bold text-[#1C1C1E]">Conversion Analysis</h1>
        <p className="text-xs text-apple-gray-500 mt-0.5">Lead-to-member conversion tracking by source.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] p-5">
          <h2 className="text-xs font-semibold text-[#1C1C1E] mb-4">Conversion by Source (Pie)</h2>
          ...
          <div className="grid grid-cols-2 gap-2 mt-4">
            {sourceData.map((d, i) => (
              <div key={d.source} className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-apple-gray-400">{d.source} ({d.rate}%)</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <h2 className="text-xs font-semibold text-[#1C1C1E] mb-3">Source-wise Conversion</h2>
          <Table
            columns={[
