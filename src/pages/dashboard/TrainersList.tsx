  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-bold text-[#1C1C1E]">My Trainers</h1>
          <p className="text-xs text-apple-gray-500 mt-0.5">Manage your trainer team (max 10).</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => setModal({ type: 'add-trainer' })} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90 transition-opacity">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatsCard label="Total Trainers" value={trainers.length} icon={UserCheck} color="from-[#007AFF]/10 to-[#007AFF]/5" border="border-[#007AFF]/20" text="text-[#007AFF]" />
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg pl-9 pr-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors" placeholder="Search by mobile, email..." />
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setModal({ type: 'bulk-addtime' })} className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-medium text-[#007AFF] bg-blue-500/10 border border-blue-500/20 rounded-lg hover:bg-blue-500/20 transition-colors">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-apple-gray-200 bg-white/[0.02] overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-apple-gray-200 bg-white/[0.03]">
                ...
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Trainer</th>
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Trainer Type</th>
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Checkin Status</th>
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Email</th>
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Phone</th>
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Star</th>
                <th className="text-left px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Status</th>
                <th className="text-right px-3 py-3 text-[10px] font-semibold text-apple-gray-500 uppercase tracking-wider">Action</th>
              </tr>
            </thead>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-apple-blue/20 to-amber-600/10 border border-ydl-yellow/20 flex items-center justify-center">
                        <span className="text-[9px] font-bold text-apple-blue">{t.name.split(' ').map(n => n[0]).join('').slice(0, 2)}</span>
                      </div>
                      <div>
                        <span className="text-xs font-medium text-[#1C1C1E] cursor-pointer hover:text-apple-blue transition-colors" onClick={() => setModal({ type: 'view-trainer', data: t })}>{t.name}</span>
                        {t.subtitle && <p className="text-[9px] text-apple-blue">{t.subtitle}</p>}
                      </div>
                    </div>
                  </td>
                  <td className="px-3 py-3">
                    <span className="text-xs text-apple-gray-400">{t.type}</span>
                    <p className="text-[9px] text-apple-gray-400">{t.id}</p>
                  </td>
                  <td className="px-3 py-3">{checkinBadge(t.checkin)}</td>
                  <td className="px-3 py-3 text-xs text-apple-gray-400">{t.email}</td>
                  <td className="px-3 py-3 text-xs text-apple-gray-400">{t.phone}</td>
                  <td className="px-3 py-3 text-xs text-apple-gray-500">{t.star}</td>
        ...
        {filtered.length === 0 && <div className="text-center py-10 text-xs text-apple-gray-500">No trainers match your search.</div>}
        ...
        {modal?.data && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-apple-blue/20 to-amber-600/10 border border-ydl-yellow/20 flex items-center justify-center">
                <span className="text-lg font-bold text-apple-blue">{modal.data.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}</span>
              </div>
              <div>
                <h3 className="text-sm font-bold text-[#1C1C1E]">{modal.data.name}</h3>
                {modal.data.subtitle && <p className="text-[10px] text-apple-blue">{modal.data.subtitle}</p>}
                <p className="text-[10px] text-apple-gray-500">{modal.data.type} ({modal.data.id})</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 text-xs">
              ...
              <div><span className="text-apple-gray-500">Check-in:</span> <span className="text-[#1C1C1E]">{modal.data.checkinTime === '-' ? 'Not checked in' : modal.data.checkinTime}</span></div>
              <div><span className="text-apple-gray-500">Status:</span> {statusBadge(modal.data.status)}</div>
              <div><span className="text-apple-gray-500">Star Trainer:</span> <span className="text-apple-gray-500">{modal.data.star}</span></div>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={modal?.type === 'edit-trainer'} onClose={() => setModal(null)} title={`Edit Trainer: ${modal?.data?.name || ''}`} size="lg">
        {editForm && (
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div className="space-y-1.5"><label className="text-[10px] text-apple-gray-500">Gender</label><select value={editForm.gender} onChange={e => setEditForm({ ...editForm, gender: e.target.value })} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40"><option>Male</option><option>Female</option><option>Other</option></select></div>
            <div className="space-y-1.5"><label className="text-[10px] text-apple-gray-500">Type</label><select value={editForm.type} onChange={e => setEditForm({ ...editForm, type: e.target.value })} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40"><option>Head Trainer</option><option>Senior Trainer</option><option>Trainer</option><option>Junior Trainer</option><option>Yoga Trainer</option></select></div>
            <div className="flex items-center gap-3 pt-2 border-t border-apple-gray-200 mt-2 col-span-2">
              <button onClick={() => { setTrainers(prev => prev.map(tr => tr.id === editForm.id ? { ...tr, ...editForm } : tr)); setModal(null) }} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90"><Edit3 className="w-3 h-3 inline mr-1" /> Save Changes</button>
      ...
      <Modal open={modal?.type === 'confirm-check'} onClose={() => setModal(null)} title={`Confirm ${modal?.data?.checkin === 'Present' || modal?.data?.checkin === 'Late' ? 'Check Out' : 'Check In'}`} size="sm">
        {modal?.data && (
          <div>
            <p className="text-xs text-apple-gray-400">Are you sure you want to <strong className="text-[#1C1C1E]">{modal.data.checkin === 'Present' || modal.data.checkin === 'Late' ? 'check out' : 'check in'}</strong> <span className="text-apple-blue">{modal.data.name}</span>?</p>
            <p className="text-[10px] text-apple-gray-500 mt-1">Timestamp: {new Date().toLocaleString('en-IN')}</p>
            <div className="flex items-center gap-3 mt-4">
              <button onClick={() => handleCheckInOut(modal.data)} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">Confirm</button>
              <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Cancel</button>
            </div>
          </div>
        )}
      ...
      <Modal open={modal?.type === 'confirm-deactivate'} onClose={() => setModal(null)} title={`Confirm ${modal?.data?.status === 'Active' ? 'Deactivate' : 'Activate'}`} size="sm">
        {modal?.data && (
          <div>
            ...
            <div className="flex items-center gap-3 mt-4">
              <button onClick={() => handleDeactivate(modal.data)} className={`px-4 py-2 text-xs font-semibold text-[#1C1C1E] rounded-lg ${modal.data.status === 'Active' ? 'bg-red-500 hover:bg-red-600' : 'bg-emerald-500 hover:bg-emerald-600'} transition-colors`}>Confirm</button>
              <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Cancel</button>
            </div>
          </div>
        )}
        ...
        {modal?.data && (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><label className="text-[10px] text-apple-gray-500">Date</label><input type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40 [color-scheme:dark]" /></div>
              <div className="space-y-1.5"><label className="text-[10px] text-apple-gray-500">Time</label><input type="time" className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40 [color-scheme:dark]" /></div>
            </div>
            <div className="flex items-center gap-3 pt-2">
              <button onClick={() => { toast(`Time added for ${modal?.data?.name}`, 'success'); setModal(null) }} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90"><Clock className="w-3 h-3 inline mr-1" /> Add Time</button>
              <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Cancel</button>
            </div>
          </div>
        )}
      ...
      <Modal open={modal?.type === 'add-trainer'} onClose={() => setModal(null)} title="Add Trainer" size="md">
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="col-span-2 space-y-1.5"><label className="text-[10px] text-apple-gray-500">Name</label><input className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40" placeholder="Trainer name" /></div>
          <div className="space-y-1.5"><label className="text-[10px] text-apple-gray-500">Email</label><input className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40" placeholder="Email" /></div>
          <div className="space-y-1.5"><label className="text-[10px] text-apple-gray-500">Phone</label><input className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40" placeholder="Phone" /></div>
          <div className="col-span-2 flex gap-3 pt-2">
            <button onClick={() => { toast('Trainer added successfully!', 'success'); setModal(null) }} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90"><Plus className="w-3 h-3 inline mr-1" /> Add</button>
            <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Cancel</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={modal?.type === 'bulk-checkin'} onClose={() => setModal(null)} title="Bulk Check In" size="sm">
        <p className="text-xs text-apple-gray-400">Check in {selectedRows.size || 'all'} selected trainer(s)?</p>
        <div className="flex items-center gap-3 mt-4">
          <button onClick={() => { if (selectedRows.size) { setTrainers(prev => prev.map(t => selectedRows.has(t.id) ? { ...t, checkin: 'Present', checkinTime: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) } : t)); setModal(null) } }} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">Confirm</button>
          <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Cancel</button>
        </div>
      </Modal>

      <Modal open={modal?.type === 'bulk-checkout'} onClose={() => setModal(null)} title="Bulk Check Out" size="sm">
        <p className="text-xs text-apple-gray-400">Check out {selectedRows.size || 'all'} selected trainer(s)?</p>
        <div className="flex items-center gap-3 mt-4">
          <button onClick={() => { if (selectedRows.size) { setTrainers(prev => prev.map(t => selectedRows.has(t.id) ? { ...t, checkin: 'Absent', checkinTime: '-' } : t)); setModal(null) } }} className="px-4 py-2 text-xs font-semibold text-black bg-apple-gradient-blue rounded-lg hover:opacity-90">Confirm</button>
          <button onClick={() => setModal(null)} className="px-4 py-2 text-xs font-medium text-apple-gray-400 bg-white/5 border border-apple-gray-200 rounded-lg hover:text-[#1C1C1E]">Cancel</button>
        </div>
      </Modal>
    </div>
