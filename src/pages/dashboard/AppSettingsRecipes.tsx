  ...
  return (
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-lg font-bold text-[#1C1C1E]">Recipes</h1><p className="text-xs text-apple-gray-500 mt-0.5">General recipes for members and trainers.</p></div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Table
          columns={[
            { header: 'Recipe Name', accessor: (r: Recipe) => <div className="flex items-center gap-2"><BookOpen className="w-3.5 h-3.5 text-apple-blue" /><span className="text-[#1C1C1E] font-medium">{r.recipeName}</span></div> },
            { header: 'Ingredients', accessor: (r: Recipe) => <span className="text-apple-gray-400 max-w-[200px] truncate block">{r.ingredients}</span> },
            { header: 'Instructions', accessor: (r: Recipe) => <span className="text-apple-gray-400 max-w-[250px] truncate block">{r.instructions}</span> },
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editRecipe ? 'Edit Recipe' : 'Add Recipe'} size="md">
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <input value={form.recipeName} onChange={e => setForm(p => ({ ...p, recipeName: e.target.value }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40" placeholder="Banana Pancakes" />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-apple-gray-400">Ingredients</label>
            <textarea value={form.ingredients} onChange={e => setForm(p => ({ ...p, ingredients: e.target.value }))} rows={2} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" placeholder="Banana, eggs, oats, cinnamon" />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-apple-gray-400">Instructions</label>
            <textarea value={form.instructions} onChange={e => setForm(p => ({ ...p, instructions: e.target.value }))} rows={3} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 resize-none" placeholder="Mash banana, mix with eggs and oats, cook on pan." />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-medium text-apple-gray-400">Status</label>
            <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as Recipe['status'] }))} className="w-full bg-white/5 border border-apple-gray-200 rounded-lg px-3 py-2 text-xs text-[#1C1C1E] focus:outline-none focus:border-ydl-yellow/40">
