    ...
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/dashboard/trainers/list')} className="p-1.5 text-gray-500 hover:text-white bg-white/5 border border-ydl-dark-border rounded-lg hover:bg-white/10 transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <h1 className="text-lg font-bold text-white">Add Trainer</h1>
            <p className="text-xs text-gray-500 mt-0.5">Register a new trainer to the system.</p>
          </div>
        </div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-5 max-w-3xl">
        <div className="flex flex-col sm:flex-row gap-6">
          <div className="flex flex-col items-center gap-3">
            <div className="w-24 h-24 rounded-xl bg-white/5 border border-ydl-dark-border flex items-center justify-center overflow-hidden">
              {photo.preview ? (
            ...
            <label className="cursor-pointer px-3 py-1.5 text-[10px] font-medium text-ydl-yellow bg-ydl-yellow/10 border border-ydl-yellow/20 rounded-lg hover:bg-ydl-yellow/20 transition-colors">
              Upload Photo
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2 space-y-1.5">
              ...
              <div className="relative">
                ...
                <input value={form.name} onChange={e => { set('name', e.target.value); if (errors.name) setErrors(prev => { const n = { ...prev }; delete n.name; return n }) }} className={`w-full bg-white/5 border ${errors.name ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors`} placeholder="Trainer full name" />
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input value={form.phone} onChange={e => { set('phone', e.target.value); if (errors.phone) setErrors(prev => { const n = { ...prev }; delete n.phone; return n }) }} className={`w-full bg-white/5 border ${errors.phone ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors`} placeholder="+91 98765 43210" />
              </div>
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-gray-400">Specialization</label>
              <div className="relative">
                <Dumbbell className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500" />
                <select value={form.specialization} onChange={e => set('specialization', e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 transition-colors appearance-none">
                  {specializations.map(o => <option key={o}>{o}</option>)}
                </select>
              </div>
            </div>
            <div className="space-y-1.5">
              <label className="text-[11px] font-medium text-gray-400">Trainer Type</label>
              <div className="relative">
                <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500" />
                <select value={form.trainerType} onChange={e => set('trainerType', e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 transition-colors appearance-none">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type="number" min={0} max={100} value={form.commission} onChange={e => set('commission', Math.max(0, Math.min(100, Number(e.target.value))))} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors" />
              </div>
              ...
              <span className="text-[10px] text-ydl-yellow font-medium">{form.commission}%</span>
            </div>
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <select value={form.status} onChange={e => set('status', e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 transition-colors appearance-none">
        ...
        <div className="flex items-center gap-3 mt-6 pt-4 border-t border-ydl-dark-border">
          ...
          <button onClick={() => navigate('/dashboard/trainers/list')} className="px-4 py-2 text-xs font-medium text-gray-400 bg-white/5 border border-ydl-dark-border rounded-lg hover:text-white transition-colors">
