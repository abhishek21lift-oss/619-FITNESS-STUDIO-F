  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        className="rounded-2xl border border-ydl-dark-border bg-ydl-card-gradient p-6 max-w-md">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 rounded-xl bg-green-500/20 text-green-400">
            <QrCode className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-white font-semibold">Enter Member Code</h2>
            <p className="text-xs text-ydl-muted">Scan QR code or type member code</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <input value={code} onChange={e => setCode(e.target.value)}
            ...
            className="flex-1 bg-[#1A1A1A] text-white border border-ydl-dark-border rounded-xl px-4 py-3 text-sm outline-none focus:border-green-500/50 uppercase" />
          <button onClick={checkIn}
            className="px-6 py-3 rounded-xl bg-green-600 text-white font-medium text-sm hover:bg-green-700 transition-all flex items-center gap-2">
      ...
      <div>
        <h2 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
          <Clock className="w-4 h-4 text-ydl-muted" /> Recent Check-Ins
        </h2>
        {loading ? (
              ...
              <tbody className="divide-y divide-ydl-dark-border">
                {recent.map((r, i) => (
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-3 text-ydl-muted">{r.memberCode}</td>
                    <td className="p-3 text-ydl-muted">{new Date(r.checkIn).toLocaleString()}</td>
                    <td className="p-3"><span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400">{r.method}</span></td>
                  </motion.tr>
                ))}
              </tbody>
            </table>
