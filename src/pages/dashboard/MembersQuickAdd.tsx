    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-5 max-w-lg">
        <div className="space-y-3">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input value={name} onChange={e => { setName(e.target.value); if (errors.name) setErrors(prev => { const n = { ...prev }; delete n.name; return n }) }} className={`w-full bg-white/5 border ${errors.name ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors`} placeholder="Full name" />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input value={mobile} onChange={e => { setMobile(e.target.value); if (errors.mobile) setErrors(prev => { const n = { ...prev }; delete n.mobile; return n }) }} className={`w-full bg-white/5 border ${errors.mobile ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors`} placeholder="+91 98765 43210" />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <select value={gender} onChange={e => setGender(e.target.value)} className="w-full bg-white/5 border border-ydl-dark-border rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 transition-colors appearance-none">
        ...
        <div className="flex items-center gap-3 mt-6 pt-4 border-t border-ydl-dark-border">
          ...
          <button onClick={() => navigate('/dashboard/members/database')} className="px-4 py-2 text-xs font-medium text-gray-400 hover:text-white transition-colors">Cancel</button>
        </div>
      </motion.div>
    </div>
  )
