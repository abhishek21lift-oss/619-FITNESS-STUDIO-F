    ...
    <div className="p-4 lg:p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/dashboard/staff/list')} className="p-1.5 text-gray-500 hover:text-white bg-white/5 border border-ydl-dark-border rounded-lg hover:bg-white/10 transition-colors">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-ydl-dark-border bg-white/[0.02] p-5 max-w-2xl">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2 space-y-1.5">
            ...
            <div className="relative">
              ...
              <input value={form.name} onChange={e => { set('name', e.target.value); if (errors.name) setErrors(prev => { const n = { ...prev }; delete n.name; return n }) }} className={`w-full bg-white/5 border ${errors.name ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors`} placeholder="Staff full name" />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input type="email" value={form.email} onChange={e => { set('email', e.target.value); if (errors.email) setErrors(prev => { const n = { ...prev }; delete n.email; return n }) }} className={`w-full bg-white/5 border ${errors.email ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-ydl-yellow/40 transition-colors`} placeholder="email@example.com" />
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <select value={form.role} onChange={e => { set('role', e.target.value); if (errors.role) setErrors(prev => { const n = { ...prev }; delete n.role; return n }) }} className={`w-full bg-white/5 border ${errors.role ? 'border-red-400' : 'border-ydl-dark-border'} rounded-lg pl-9 pr-8 py-2 text-xs text-white focus:outline-none focus:border-ydl-yellow/40 transition-colors appearance-none`}>
        ...
        <div className="flex items-center gap-3 mt-6 pt-4 border-t border-ydl-dark-border">
          ...
          <button onClick={() => navigate('/dashboard/staff/list')} className="px-4 py-2 text-xs font-medium text-gray-400 bg-white/5 border border-ydl-dark-border rounded-lg hover:text-white transition-colors">
