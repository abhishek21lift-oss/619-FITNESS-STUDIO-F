    ...
    <div className="p-6 space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { label: 'My Members', value: stats?.totalMembers || 0, icon: Users, color: 'from-blue-500/20 to-blue-600/10 text-blue-400 border-blue-500/30' },
            ...
            { label: "Today's Classes", value: stats?.todayClasses || 0, icon: CalendarCheck, color: 'from-ydl-yellow/20 to-amber-600/10 text-ydl-yellow border-ydl-yellow/30' },
          ].map((c, i) => (
            <motion.div key={c.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <div className="flex items-start justify-between">
                <div><p className="text-sm text-ydl-muted">{c.label}</p><p className={`text-2xl font-bold mt-1 ${c.color.split(' ')[2]}`}>{c.value}</p></div>
